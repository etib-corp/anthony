...
- **Dependency strategy**: FetchContent
...
- `headers/` — ANTHony public headers under `ANTHony/`
- `sources/` — ANTHony implementation sources
- `tests/` — ANTHony unit tests
...
- **Header/source placement**:
  - Public headers: `headers/ANTHony/` (e.g., `headers/ANTHony/Core.hpp`)
...
- **Naming**:
  ...
  - Namespaces: nested by module (e.g., `ANTHony`, `ANTHony::builder`)
...
- **Edit with care**:
  - Public headers in `headers/ANTHony/` — update docs/tests if API changes
...
2. **Add a source/header pair to an existing module**: Place `.hpp` in `headers/ANTHony/` and `.cpp` in `sources/`, follow naming conventions (`PascalCase` class, `camelCase` methods, `_` prefix for members), update namespace nesting, verify build.
